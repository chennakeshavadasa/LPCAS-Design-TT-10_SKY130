...
S {}
E {}
B 2 1480 -720 2170 -170 {flags=graph,unlocked
y1=1.632
y2=1.692
ypos1=0
ypos2=2
...
node=vreg
hcursor1_y=1.668658
hcursor2_y=1.6358247
sim_type=op}
B 2 2230 -720 2790 -170 {flags=graph,unlocked
y1=1.6
y2=1.7
ypos1=0
ypos2=2
...
node=vdiff
sweep=f1}
P 15 7 1850 -100 1850 -130 1900 -130 1900 -140 1920 -130 1900 -120 1900 -130 {}
T {tcleval(mean=[to_eng $mean]
variance=[to_eng $variance]
rms=[to_eng $rms])} 1930 -140 0 0 0.4 0.4 {floater=yes layer=6}
T {The shown mean is affected by
non converging points
which are extreme outliers} 1550 -90 0 0 0.4 0.4 {layer=15}
...
C {sky130_fd_pr/corner.sym} 140 -160 0 0 {name=CORNER1 only_toplevel=true corner=tt_mm}
C {devices/launcher.sym} 190 -20 0 0 {name=h15
descr="Annotate OP" 
tclcommand="
...
C {sky130_fd_pr/nfet_05v0_nvt.sym} 520 -440 0 0 {name=M1
W=1
L=0.9
nf=1
mult=8
...
C {sky130_fd_pr/nfet_05v0_nvt.sym} 790 -440 0 1 {name=M2
W=1
L=0.9
nf=1
mult=8
...
C {code.sym} 10 -150 0 0 {name=NGSPICE only_toplevel=false value="
.options savecurrents itl2=100 method=gear KEEPOPINFO
.control
 let runs=1000
 let r=0
 setseed 23
...
.endc
"}
C {devices/launcher.sym} 1660 -120 0 0 {name=h5
descr="load waves" 
tclcommand="
xschem raw_read $netlist_dir/LDO4_stef_MC.raw op
proc mean \{\} \{
  set sum 0
  set points [xschem raw points]
  # -1 means return all OP points   ..
  foreach i [xschem raw values vreg -1] \{
    set sum [expr \{$sum + $i\}]
    puts $i
  \}
...
C {devices/launcher.sym} 2270 -100 0 0 {name=h1
descr="Distrib" 

tclcommand="
proc get_histo \{\} \{
  set min [expr \{0.\}] ;# this has exact floating point notation
  set max [expr \{1.8\}]
  proc xround \{a size\} \{ return [expr \{round($a/$size) * $size\}]\}
  #### get rounded data
  set size [expr \{$max / 2048\}]
  catch \{unset f1\}
  foreach v1 [xschem raw values vreg -1] \{
  ...
  for \{set i $min\} \{$i <= $max\} \{set i [expr \{$i + $size\}] \} \{
    set v1 0
    set ii [xround $i $size]
    if \{[info exists f1($ii)]\} \{ set v1 $f1($ii) \}
    xschem raw set f1 $j $v1
    incr j
  \}
\}
get_histo
# draw graph 1 (the second box on the right) with raw file 1 (distrib created above)
xschem draw_graph 1
# switch back to first loaded raw file (raw file 0), that is OP data
# so graph on the left displays OP data.
xschem raw switch 0
"
}
C {res.sym} 1310 -530 0 0 {name=R14
value=500
footprint=1206
device=resistor
m=1}
